...
class Slot {
    ...
    convertPositionToSymbols(positions) {
        let symbols = [];
    ...
    getCountOfEachElementinArray(index, symbols) {
        let lineSymbols = {};
        for (let j = 0; j < defaults.lines[index].length; j++) {
            let lineSymbol = symbols[j][defaults.lines[index][j]]; //takes a specipic element for the lines
    ...
    checkWinnings(lineSymbols, linesPayout) {
    ...
    spin() {
        let linesPayout = [];
        let totalWinnings = 0;
        let positions = this.positionGenerator();
        let symbols = this.convertPositionToSymbols(positions);
